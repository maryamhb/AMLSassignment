...
In order to train a suitable model for the required classification tasks, several preprocessing methods were taken into consideration to provide appropriate features for the process; for instance, facial landmarks were extracted via various detection methods, %using Python's Dlib and Open Source Computer Vision (OpenCV) libraries; 
namely the Histogram of Gradients (HoG) Face Detector% accessible via Dlib
, Haar Cascade Face Detector% accessible via OpenCV
, and the Deep Learning-based Face Detector%, which can be implementable via both libraries
